...
export default function EmployeeList() {
  ...
  const handleSearch = (e) => {
    e.preventDefault()
    dispatch(getAllEmployees({ ...search, page: 1, limit }))
    setCurrentPage(1)
  }
  ...
  useEffect(() => {
    if (isDeleted) {
      dispatch(getAllEmployees({ page: currentPage }))
  ...
  useEffect(() => {
    dispatch(getAllEmployees({ ...search, page: currentPage }))
  ...
  const error = updateError || createError || getLoading
  // console.log('First employee row:', employees?.managers[0])
  ...
  return loading ? (
    ...
    <>
      ...
      {error && <p>{error}</p>}
      <DataTable
        title={`Employee (${(currentPage - 1) * limit + 1}–${Math.min(currentPage * limit, employees.totalManagers)} of ${employees.totalManagers})`}
        columns={['S.No', 'Email', 'Username', 'ContactNumber', 'Status', 'Delete', 'Edit']}
        ...
        renderRow={(row, index) => [
          ...
          row.role !== Constants.ROLES.ADMIN && (
            <Button
              title="Delete"
              color="danger"
              size="sm"
              onClick={() => {
                setStateToDelete(row._id)
